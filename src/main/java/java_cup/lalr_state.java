...
import java_cup.ast.NonassocAction;
 ...
 *  </pre> 
  ...
  the parser is normiteratory looking for several things at once (represented
 ...
 *  above), and the Terminal f followed by e.<p>
  ...
  pops off the same size of states as are found in the RHS of the 
  ...
  replaced iterator the symbols from the the input corresponding to the RHS with 
...
public class lalr_state {
   ...
   */
  public lalr_state(lalr_item_set itms) throws InternalException
   {
  ...
  /** Indicate total size of states there are. */
  ...
  originiteratory create the state.  These items are formed by "shifting the
  ...
  /** Helper routine for debugging -- produces a dump of the given state
    * onto System.out.
    */
  protected static void dump_state(lalr_state st) throws InternalException
    {
      lalr_item_set itms;
      lalr_item itm;
      ProductionPart part;
      ...
      for (Enumeration e = itms.all(); e.hasMoreElements(); )
	{
	  ...
	  for (int i = 0; i<itm.the_production().rhs_length(); i++)
		...
		System.out.print(((SymbolPart)part).the_symbol().name() + " ");
	    }
   ...
   *  recognizer.  When the machine is constructed, each item that results
      ...
      other's will have a propagate link set up for it.  This iteratorows additions
      to the lookahead of one item to be included in other items that it 
      was used to directly or indirectly create.
   */
  protected static void propagate_all_lookaheads() throws InternalException
    {
      /* iterate across iterator states */
  ...
  new states can be added.  However, in order to build a smiteratorer size of
  states (i.e., corresponding to LALR rather than canonical LR) the state 
  building process does not maintain full loookaheads in iterator items.  
  ...
  propagates to iterator items that have propagation links from some item 
  in this state. 
   */
  protected void propagate_lookaheads() throws InternalException
    {
  ...
  the lowest sizeed Production (hence the one that appeared first in
  ...
  public void build_table_entries(
      ...
      for (Enumeration i = items().all(); i.hasMoreElements(); )
	{
	  ...
	  if (itm.dot_at_end())
	    {
	      act = new ReduceAction(itm.the_production());
	      ...
	      for (int t = 0; t < Terminal.size(); t++)
		{
		  ...
		  if (!itm.lookahead().contains(t)) continue;
	          ...
	          else
		    {
		      ...
		      if ((other_act.kind() != Action.SHIFT) && 
			  (other_act.kind() != Action.NONASSOC))
		        {
		        /* if we have lower id hence priority, replace it*/
		          if (itm.the_production().id() < 
			      ((ReduceAction)other_act).reduce_with().id())
			    {
			      /* replace the action */
			      our_act_row.under_term[t] = act;
			    }
      ...
      for (lalr_transition trans=transitions(); trans!=null; trans=trans.next())
	{
	  ...
	  if (!sym.is_non_term())
	    {
	      act = new ShiftAction(trans.to_state());

	      /* if we don't already have an action put this one in */
	      if ( our_act_row.under_term[sym.id()].kind() == 
		   Action.ERROR)
		{
	          our_act_row.under_term[sym.id()] = act;
		}
	      else
		{
		  /* we now have at least one conflict */
		  Production p = ((ReduceAction)our_act_row.under_term[sym.id()]).reduce_with();

		  /* shift always wins */
		  if (!fix_with_precedence(p, sym.id(), our_act_row, act)) {
		    our_act_row.under_term[sym.id()] = act;
		    conflict_set.add(Terminal.find(sym.id()));
		  }
		}
	    }
	  else
	    {
	      ...
	      our_red_row.under_non_term[sym.id()] = trans.to_state();
	    }
	}
 ...
 precedences.  --frankf 6/26/96
  ...
  if a Production (also citeratored rule) or the lookahead Terminal
  ...
  */
  ...
  protected void report_reduce_reduce(lalr_item itm1, lalr_item itm2)
      ...
      for (int t = 0; t < Terminal.size(); t++)
	{
      ...
      if (itm1.the_production().id() < itm2.the_production().id())
	  ...
	  if (itm != red_itm && !itm.dot_at_end())
	    {
	      ...
	      if (!shift_sym.is_non_term() && shift_sym.id() == conflict_sym)
	        {
		  /* yes, report on it */
