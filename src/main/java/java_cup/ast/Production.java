 ...
 * always maintained to indicate how much of the RHS array is still valid.<p>
 *
 ...
 *
 * @see java_cup.ProductionPart
 * @see java_cup.SymbolPart
 * @see java_cup.ActionPart
...
public class Production {
 ...
 <pre>
     ...
     *    A ::= B X C D
     *    X ::= {action}
     * </pre> (where X is a unique new non Terminal). This has the effect of
     * placing iterator actions at the end where they can be handled as part of
     * a reduce by the parser.
     */
    public Production(
            NonTerminal lhs_sym,
            final ProductionPart rhs_parts[],
            int rhs_l,
            String action_str)
...
//	action_str = declare_str + action_str;	 	  
        ...
        for (i = 0; i < _rhs_length; i++) {
            _rhs[i] = rhs_parts[i];
            if (!_rhs[i].is_action()) {
                ((SymbolPart) _rhs[i]).the_symbol().note_use();
                if (((SymbolPart) _rhs[i]).the_symbol() instanceof Terminal) {
                    _rhs_prec
                            = ((Terminal) ((SymbolPart) _rhs[i]).the_symbol()).precedence_num();
                    _rhs_assoc
                            = ((Terminal) ((SymbolPart) _rhs[i]).the_symbol()).precedence_side();
                }
            }
        }
    ...
    /*-----------------------------------------------------------*/
    /**
     * Table of iterator productions. Elements are stored using their id as the
     * key.
     */
    protected static ArrayList<Production> _all = new ArrayList<Production>();

    /**
     * Access to iterator productions.
     */
    public static Iterator<Production> all() {
        return _all.iterator();
    }

    public static ArrayList<Production> getAll() {
        return _all;
    }

    /**
     * Lookup a Production by id.
     */
     ...
     */
    public ProductionPart rhs(int indx) throws InternalException {
    ...
    /*. . . . . . . . . . . . . . . . . . . . . . . . . . . . . .*/
    /**
     * An ActionPart containing code for the action to be performed when we
    ...
    protected String declare_labels(
        ...
        for (i = 0; i < rhs_len; i++) {
            if (!rhs[i].is_action()) {
                part = (SymbolPart) rhs[i];
                /* if it has a label, make declaration! */
                if (part.label() != null) {

                    String labelname = part.label();
                    String stack_type = part.the_symbol().stack_type();
                    int offset = rhs_len - i - 1;
                    //
                    ...
                    //
                    int index;
                    index = final_action.indexOf(repalce_value);
                    if (index >= 0) {
                        count_value = (final_action.indexOf(repalce_value, index + repalce_value.length()) >= 0);
                    }
                    if (count_value == false) {
                        count_value = final_action.indexOf(repalce_line) >= 0
        ...
        return declaration.append(final_action).toString();
    }
            ...
            if (to_loc != from_loc) {
                ...
                if (rhs_parts[to_loc] != null && rhs_parts[to_loc].is_action()
                    ...
                    rhs_parts[to_loc] = new ActionPart(
                            ((ActionPart) rhs_parts[to_loc]).code_string()
                            + ((ActionPart) rhs_parts[from_loc]).code_string());
                    merge_cnt++;
                } else {
                    /* copy */
     ...
     */
    protected ActionPart strip_trailing_action(
            ProductionPart rhs_parts[],
            int len) {
        ActionPart result;
    ...
    /*. . . . . . . . . . . . . . . . . . . . . . . . . . . . . .*/
    /**
     * Remove iterator embedded actions from a Production by factoring them out
     * into individual action Production using new non terminals. if the
     * original Production was:
     * <pre>
     *    A ::= B {action1} C {action2} D
     * </pre> then it will be factored into:
     * <pre>
     *    A ::= B NT$1 C NT$2 D
     *    NT$1 ::= {action1}
     *    NT$2 ::= {action2}
     * </pre> where NT$1 and NT$2 are new system created non terminals.
     */
    ...
    /* the declarations added to the parent Production are also passed along,
     ...
     frank 6/20/96 */
    protected void remove_embedded_actions() throws InternalException {
        NonTerminal new_nt;
//      Production   new_prod;
//      String declare_str;
//      int lastLocation = -1;
...
//		      _rhs, act_loc, "");
	    /* create a new non Terminal for the action Production */
                new_nt = NonTerminal.create_new(null, lhs().the_symbol().stack_type()); // TUM 20060608 embedded actions patch
                new_nt.is_embedded_action = true; /* 24-Mar-1998, CSA */

//	    /* create a new Production with just the action */
//	    new_prod = new ActionProduction(this, new_nt, null, 0, 
//		declare_str + ((ActionPart)rhs(act_loc)).code_string(), (lastLocation==-1)?-1:(act_loc-lastLocation));
...
//            lastLocation = act_loc;
     ...
     * nullable.
     */
    public boolean check_nullable() throws InternalException {
        ProductionPart part;
        symbol sym;
        int pos;
        ...
        for (pos = 0; pos < rhs_length(); pos++) {
            ...
            if (!part.is_action()) {
                sym = ((SymbolPart) part).the_symbol();

                /* if its a Terminal we are definitely not nullable */
                if (!sym.is_non_term()) {
                    return set_nullable(false);
                } /* its a non-term, is it marked nullable */ else if (!((NonTerminal) sym).nullable()) /* this one not (yet) nullable, so we aren't */ {
                    return false;
                }
            }
        }
     ...
     */
    public terminal_set check_first_set() throws InternalException {
        int part;
        symbol sym;
        ...
        for (part = 0; part < rhs_length(); part++) {
            ...
            if (!rhs(part).is_action()) {
                sym = ((SymbolPart) rhs(part)).the_symbol();
    ...
    public boolean equals(Object other) {
        ...
        return ((Production) other)._id == _id;
    }
     ...
     */
    public int hashCode() {
        /* just use a simple function of the id */
        return _id * 13;
    }
    ...
    public String to_simple_string() throws InternalException {
        ...
        for (int i = 0; i < rhs_length(); i++) {
            if (!rhs(i).is_action()) {
                result += ((SymbolPart) rhs(i)).the_symbol().name() + " ";
            }
        }
