...
public class lalr_item extends lr_item_core {
   ...
   * @param look the set of lookahead symbols.
   */
  public lalr_item(Production prod, terminal_set look) throws InternalException
    {
      this(prod,0,look);
    }
   ...
   * @param prod the Production for the item.
   */
  public lalr_item(Production prod) throws InternalException
    {
   ...
   */
  public void propagate_lookaheads(terminal_set incoming) throws InternalException
    {
   ...
   *  to the right. 
   */
  public lalr_item shift() throws InternalException
    {
      lalr_item result;
      ...
      for (pos = dot_pos()+1; pos < the_production().rhs_length(); pos++) 
	{
	   ...
	   if (!part.is_action())
	       ...
	       else
		 {
		   ...
		   result.add(((NonTerminal)sym).first_set());
   ...
   *  iterator items produced as a closure of this item.  Note: this routine should 
   *  not be invoked until after first sets and nullability have been 
   *  calculated for iterator non terminals. 
   */
  public boolean lookahead_visible() throws InternalException
    {
      ProductionPart part;
      symbol          sym;
